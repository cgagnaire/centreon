...
sub new {
    ...
    bless $self, $class;
    $self->add_options(
        "p|poller=s" => \$self->{opt_p},
        "s|startdate=s" => \$self->{opt_s}
        );
...
sub getPollerId {
    ...
    my ($status, $sth) = $self->{cdb}->query("SELECT id 
                                              ...
                                              LIMIT 1");
    if ($status == -1) {
        $self->{logger}->writeLogError("Could not get poller id");
        return 0;
    }
...
sub parseFile {
    my $self = shift;
    my ($log_file, $instance_name) = @_;
    my @log_table_rows;

    # Open Log File for parsing
    if (!open (FILE, $log_file)){
        $self->{logger}->writeLogError("Cannot open file : $log_file");
        return;
    }
    $self->{logger}->writeLogInfo("Processing file $log_file");
...
sub parseArchive {
    ...
    if ($status == -1) {
        $self->{logger}->writeLogInfo("Could not get information on poller");
        return;
    }
    ...
    if ($tmp_server->{'localhost'}) {
        ($status, $sth) = $self->{cdb}->query("SELECT `log_archive_path`
                                               ...
                                               LIMIT 1");
        if ($status == -1) {
            $self->{logger}->writeLogError("Could not get information on poller");
            return;
        }
        my $data = $sth->fetchrow_hashref();
        if (!$data->{'log_archive_path'}) {
            $self->{logger}->writeLogError("Could not find local var log directory");
            return;
        }
    ...
    opendir (DIR, $archives) or $self->{logger}->writeLogError("Could not find $archives directory") and return;
...
sub run {
    ...
    if ($status == -1) {
        $self->{logger}->writeLogError("Could not get logs retention duration");
        return;
    }
    ...
    if ($status == -1) {
        $self->{logger}->writeLogError("Could not get host cache");
        return;
    }
    ...
    if ($status == -1) {
        $self->{logger}->writeLogError("Could not get service cache");
        return;
    }
    while ((my $tmp_cache = $sth_cache->fetchrow_hashref())) {
    ...
    ($status, $sth_cache) = $self->{cdb}->query("SELECT host.host_name, host.host_id, service.service_id, service.service_description 
                                                 ...
                                                 AND service.service_activate = '1'");
    if ($status == -1) {
        $self->{logger}->writeLogError("Could not get service by hostgroup cache");
        return;
    }
    while ((my $tmp_cache = $sth_cache->fetchrow_hashref())) {
        $self->{cache_host_service}{$tmp_cache->{'host_name'} . ':' . $tmp_cache->{'service_description'}} = {'host_id' =>  $tmp_cache->{'host_id'}, 'service_id' =>  $tmp_cache->{'service_id'}};
	...
	$self->{logger}->writeLogInfo("Processing poller $self->{opt_p}");
        ...
        if ($status == -1) {
            $self->{logger}->writeLogError("Could not get poller list");
            return;
        }
