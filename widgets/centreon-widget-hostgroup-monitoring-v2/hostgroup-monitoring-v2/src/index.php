...
require_once $centreon_path . 'www/widgets/hostgroup-monitoring-v2/src/class/HostgroupMonitoring.class.php';
...
$path = $centreon_path . "www/widgets/hostgroup-monitoring-v2/src/";
$template = new Smarty();
...
// droping empty hostgroups (where severity filter didn't match any hosts)
foreach ($data as $hostgroupName => &$properties) {
    if (empty($properties['host_state'])) {
        unset($data[$hostgroupName]);
    }
}

$critData = $criticality->getList(null, "level", "ASC", null, null, false);
...
if ($detailMode === true) {
    foreach ($data as $hostgroupName => &$properties) {
        foreach ($properties['host_state'] as $hostName => &$hostProperties) {
            ...
            if (isset($hostProperties['criticality_id']) && $hostProperties['criticality_id']) {
                $hostProperties['criticality_icon'] = $media->getFilename($critData[$hostProperties['criticality_id']]['icon_id']);
                $hostProperties['criticality_name'] = $critData[$hostProperties['criticality_id']]['hc_name'];
            }
        }
